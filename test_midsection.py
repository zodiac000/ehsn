...
"""
    ...
    def onMouseOverAx2(self, event):
        ...
        if event.inaxes == self.ax:
            ...
            self.midsecHeader.fill_ax2_mouse_over(x, y, self.tags, self.depths, self.tagmarkLineList, self.depthTagList, self.depthList)
    ...
    def onMouseClickAx2(self, event):
        ...
        if event.inaxes == self.ax:
            x, y = event.xdata, event.ydata
            self.midsecHeader.fill_ax2_click(x, y, self.tags, self.depths, self.tagmarkLineList, self.depthTagList, self.depthList)
            self.Layout()
            self.Refresh()
            # print("{}, {}".format(x, y))
